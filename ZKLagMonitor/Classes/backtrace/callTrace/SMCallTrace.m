...
+ (void)save {
    ...
    for (SMCallTraceTimeCostModel *model in arr) {
        //记录方法路径
        model.path = [NSString stringWithFormat:@"%@[%@ %@]",(model.isClassMethod? @"+":@"-"),model.className,model.methodName];
        [self appendRecord:model to:mStr];
    }
//    [[NEMonitorFileManager shareInstance] saveLastClsCallModels];
}
...
//    [mStr appendFormat:@"%@\n path%@\n",[cost des],cost.path];
    if (cost.subCosts.count < 1) {
        cost.lastCall = YES;
        //记录到数据库中
//        [[NEMonitorFileManager shareInstance] addWithClsCallModel:cost];
    } else {
        for (SMCallTraceTimeCostModel *model in cost.subCosts) {
            ...
            //记录方法的子方法的路径
            model.path = [NSString stringWithFormat:@"%@ \n\n %@[%@ %@]",cost.path,(model.isClassMethod? @"+":@"-"),model.className,model.methodName];
            [self appendRecord:model to:mStr];
        }
    }
